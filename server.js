...
// ============ CONEXIÓN A MySQL (Configurada para Railway) ============
let pool;

// Usar DATABASE_URL de Railway (formato: mysql://root:password@host:port/database)
...
console.log(`   Puerto: ${parseInt(process.env.MYSQLPORT || '3306')}`);
console.log(`   Base de datos: ${process.env.MYSQLDATABASE || 'railway'}`);
    ...
    try {
        ...
        await db.query(`
            CREATE TABLE IF NOT EXISTS users (
                ...
                role ENUM('Comprador', 'Vendedor', 'Administrador') NOT NULL,
        ...
        if (admins.length === 0) {
            // Hash para "Admin123*" (bcrypt)
            const adminPasswordHash = '$2a$10$N9qo8uLOickgx2ZMRZoMy.Mr/.b7TJqFzYI8Zz5KqKqKqKqKqKqK';
            await db.query(`
                INSERT INTO users (role, numeroControl, nombreCompleto, email, password, isActive, createdAt)
                VALUES ('Administrador', 'ADMIN001', 'Administrador ByteSnack', 'admin@bytesnack.com', ?, TRUE, NOW())
            `, [adminPasswordHash]);
...
app.listen(PORT, () => {
    ...
    console.log(`🔗 URL pública: ${process.env.RAILWAY_PUBLIC_DOMAIN || 'No disponible'}`);
});